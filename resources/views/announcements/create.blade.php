                ...
                <div class="p-8">
                    <form action="{{ route('announcements.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title') }}" placeholder="{{ __('What\'s the news?') }}" class="w-full px-4 py-3 rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm transition-all text-gray-900 font-medium placeholder:text-gray-400 @error('title') border-red-500 @enderror" required>
                            @error('title')
                        ...
                        <div>
                            ...
                            <textarea name="content" id="content" rows="12" placeholder="{{ __('Write your announcement detail here...') }}" class="w-full px-4 py-3 rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm transition-all text-gray-900 @error('content') border-red-500 @enderror" required>{{ old('content') }}</textarea>
                            @error('content')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                            <div class="mt-2 text-xs text-gray-400 flex items-center justify-between">
                                ...
                                <span id="charCount">0 {{ __('characters') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50/50 p-6 rounded-2xl border border-gray-100/50 flex items-center justify-between">
                            <div class="flex items-center">
                                <div class="flex items-center h-5">
                                    <input type="checkbox" name="is_published" id="is_published" value="1" class="w-5 h-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 transition-all cursor-pointer" {{ old('is_published') ? 'checked' : '' }}>
                                </div>
                                <div class="ml-3 text-sm">
                                    <label for="is_published" class="font-bold text-gray-900 cursor-pointer">{{ __('Publish Immediately') }}</label>
                            ...
                            <div class="hidden sm:block">
                                <div x-data="{ open: false }" class="relative">
                                    <span class="inline-flex rounded-full px-3 py-1 text-xs font-medium bg-amber-100 text-amber-800" x-show="!$refs.publishCheck.checked">
                                        {{ __('Status: Draft') }}
                                    </span>
                                    <span class="inline-flex rounded-full px-3 py-1 text-xs font-medium bg-green-100 text-green-800" x-show="$refs.publishCheck.checked">
                                        {{ __('Status: Published') }}
                                    </span>
                                </div>
                            </div>
