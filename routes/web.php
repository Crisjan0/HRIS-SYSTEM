...
Route::middleware(['auth', 'account.active', 'approved', 'password.changed'])->group(function () {
    ...
    Route::patch('employees/{employee}/restore', [EmployeeController::class, 'restore'])->name('employees.restore')->middleware('role:ADMIN,HRSTAFF,DIRECTOR');
    ...
    Route::get('employees/filter', [EmployeeController::class, 'filter'])->name('employees.filter')->middleware('role:ADMIN,HRSTAFF,DIRECTOR');
    ...
    Route::get('leave-applications/filter', [LeaveApplicationController::class, 'filter'])->name('leave-applications.filter')->middleware('role:ADMIN,HRSTAFF,DIRECTOR');
    Route::get('leave-applications/all/filter', [LeaveApplicationController::class, 'allFilter'])->name('leave-applications.all.filter')->middleware('role:ADMIN,HRSTAFF,DIRECTOR');
    ...
    Route::get('leave-applications/{leaveApplication}/print', [LeaveApplicationController::class, 'print'])->name('leave-applications.print')->middleware('role:ADMIN,HRSTAFF,DIRECTOR');
    ...
    Route::get('/announcements/filter', [AnnouncementController::class, 'filter'])->name('announcements.filter')->middleware('role:ADMIN,HRSTAFF,REGIONALDIRECTOR,REGIONAL DIRECTOR');
    ...
    Route::put('/travel-orders/{travelOrder}/status', [TravelOrderController::class, 'updateStatus'])->name('travel-orders.update-status')->middleware('role:CHIEF,HRSTAFF,ADMIN,REGIONALDIRECTOR,REGIONAL DIRECTOR,DIRECTOR');

    // Compensatory Time-Off Routes
    Route::get('/hr/cto', [CtoController::class, 'adminIndex'])->name('hr.cto.index')->middleware('role:ADMIN,HRSTAFF,CHIEF,DIRECTOR,REGIONALDIRECTOR,REGIONAL DIRECTOR');
    Route::put('/cto/{ctoRequest}/status', [CtoController::class, 'updateStatus'])->name('cto.update-status')->middleware('role:CHIEF,HRSTAFF,ADMIN,DIRECTOR,REGIONALDIRECTOR,REGIONAL DIRECTOR');
});
